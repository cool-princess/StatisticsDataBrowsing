...
@section('content')
        ...
        <div class="container">
            ...
            <div class="wrap">
                <div class="wrapper active">
                    ...
                    <div class="mainbar">
                        <form method="POST" action="{{ route('mailUpdatePost') }}">
                            @csrf
                            <div class="main-body">
                                ...
                                <div class="mail-create-part">
                                    ...
                                    <div class="mail-content-body">
                                        <input type="text" name="id" value="{{ $mails[0]->id }}" style="display: none;">
                                        ...
                                        <div class="mail-content-item">
                                            ...
                                            <div class="mail-content-item-right">
                                                <input type="text" id="title" placeholder="メールタイトル" name="title" value="{{ $mails[0]->title }}" required>
                                            </div>
                                        </div>
                                        <div class="mail-content-item">
                                            <div class="mail-content-item-left">
                                            ...
                                            <div class="mail-content-item-right">
                                                <textarea name="body" id="body" value="{{ $mails[0]->body }}" placeholder="[com]&#10;&#10;[unit]&#10;&#10;[name]様&#10;&#10;登録されたID・パスワードは以下になります。&#10;&#10;ID=[id]&#10;&#10;PW=[pw]">{{ $mails[0]->body }}</textarea>
                                            </div>
                                        </div>
                                        <div class="mail-content-item">
                                            <div class="mail-content-item-left">
                                ...
                                <div class="admin-list-part">
                                    ...
                                    <div class="mail-list-filter">
                                        <div class="mail-list-select-filter">
                                            ...
                                            <select name="prefecture" id="mail-prefecture-select">
                                            ...
                                            <select name="industry" id="mail-industry-select">
                                    ...
                                    <div class="admin-list-body">
                                        <table>
                                            ...
                                            @forelse($users as $user)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <label>
                                                            <input type="checkbox" name="user_mail[]" value="{{ $user }}" class="mail-individual-select" id="mail_select{{$i}}">
                                                            @forelse($mails[0]->user_id as $user_id)
                                                                @if($user->user_id == $user_id)
                                                                    <script>
                                                                        $( "#mail_select{{$i}}" ).prop( "checked", true );
                                                                    </script>
                                                                @endif
                                                            @empty
                                                            @endforelse
                                        ...
                                        <div class="mail-select-btn-group">
                                            <button type="submit" name="action" class="mail-select-btn btn-danger" value="delete">登録を削除</button>
                                            @if($mails[0]->delivered != '送')
                                                <button type="submit" name="action" class="mail-select-btn btn-primary" value="modify">送信・予約</button>
                                                <button type="submit" name="action" class="mail-select-btn btn-extra" value="save">保存</button>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                        ...
                        <form method="get" action="{{ route('userMailUpdateSearchGet', ['id' => $mails[0]->id]) }}" id="searchForm">
                    ...
                    <div id="code-modal" class="modal">
                        <div class="modal-content">
                            ...
                            <div class="modal-part">
                                ...
                                <div class="modal-part-item"><div class="modal-part-left">業種</div><div class="modal-part-middle">=</div><div class="modal-part-right">[ind]</div></div>
                            </div>
    ...
    <script>
        ...
        $("#year").change(function() {
            window.localStorage.setItem('year', $("#year").val());
        });
        $("#month").change(function() {
            window.localStorage.setItem('month', $("#month").val());
        });
        $("#day").change(function() {
            window.localStorage.setItem('day', $("#day").val());
        });
        $("#hour").change(function() {
            window.localStorage.setItem('hour', $("#hour").val());
        });
        $("#minute").change(function() {
            window.localStorage.setItem('minute', $("#minute").val());
        });
        $("#mail-prefecture-select").change(function() {
            window.localStorage.setItem('prefecture', $("#mail-prefecture-select").val());
        });
        $("#mail-industry-select").change(function() {
            window.localStorage.setItem('industry', $("#mail-industry-select").val());
        });
        $("#mail-freeword-select").change(function() {
            window.localStorage.setItem('freeword', $("#mail-freeword-select").val());
        });
        ...
        $(document).ready(function() {
            document.getElementById("year").value = "{{ \Carbon\Carbon::parse($mails[0]->reserve_date)->year}}";
            document.getElementById("month").value = "{{ \Carbon\Carbon::parse($mails[0]->reserve_date)->month}}";
            document.getElementById("day").value = "{{ \Carbon\Carbon::parse($mails[0]->reserve_date)->day}}";
            document.getElementById("hour").value = "{{ \Carbon\Carbon::parse($mails[0]->reserve_date)->hour}}";
            document.getElementById("minute").value = "{{ \Carbon\Carbon::parse($mails[0]->reserve_date)->minute}}";
            ...
            $('#day').val(window.localStorage.getItem('day'));
            $('#hour').val(window.localStorage.getItem('hour'));
            $('#minute').val(window.localStorage.getItem('minute'));
            $('#mail-prefecture-select').val(window.localStorage.getItem('prefecture'));
            $('#mail-industry-select').val(window.localStorage.getItem('industry'));
            $('#mail-freeword-select').val(window.localStorage.getItem('freeword'));
